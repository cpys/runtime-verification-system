...
Z3_lbool Z3_API
Z3_compute_interpolant(Z3_context c, Z3_ast pat, Z3_params p, Z3_ast_vector *out_interp, Z3_model *model) {
	Z3_TRY ;
...
void
Z3_write_interpolation_problem(Z3_context ctx, unsigned num, Z3_ast *cnsts, unsigned *parents, const char *filename,
                               unsigned num_theory, Z3_ast *theory) {
	std::ofstream f(filename);
	if (num > 0) {
#if 0
		// Suggested shorthand:
...
int Z3_read_interpolation_problem(Z3_context ctx, unsigned *_num, Z3_ast *cnsts[], unsigned *parents[],
                                  const char *filename, Z3_string_ptr error, unsigned *ret_num_theory,
                                  Z3_ast *theory[]) {
