...
struct z3_replayer::imp {
	...
	enum value_kind {
		INT64, UINT64, DOUBLE, STRING, SYMBOL, OBJECT, UINT_ARRAY, INT_ARRAY, SYMBOL_ARRAY, OBJECT_ARRAY, FLOAT
	};
	...
	void check_arg(unsigned pos, value_kind k) const {
		if (pos >= m_args.size()) {
			TRACE("z3_replayer",
			      tout << "too few arguments " << m_args.size() << " expecting " << kind2string(k) << "\n";);
			throw z3_replayer_exception("invalid argument reference");
		}
	...
	void read_ptr() {
		if (!(('0' <= curr() && curr() <= '9') || ('A' <= curr() && curr() <= 'F') ||
		      ('a' <= curr() && curr() <= 'f'))) {
...
#define TICK_FREQUENCY 100000
	...
	void **get_obj_array(unsigned pos) const {
		...
		for (unsigned i = 0; i < v.size(); i++) tout << v[i] << " ";
		tout << "\n";);
		return v.c_ptr();
	}
