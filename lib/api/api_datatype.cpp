...
void Z3_API Z3_mk_datatypes(Z3_context c,
	...
	Z3_TRY ;
		...
		for (unsigned i = 0; i < num_sorts; ++i) {
			...
			datas.push_back(
					mk_datatype_decl(c, sort_names[i], cl->size(), reinterpret_cast<Z3_constructor *>(cl->c_ptr())));
		}
...
Z3_func_decl Z3_API Z3_get_tuple_sort_mk_decl(Z3_context c, Z3_sort t) {
	Z3_TRY ;
		...
		datatype_util &dt_util = mk_c(c)->dtutil();
		if (!dt_util.is_datatype(tuple) || dt_util.is_recursive(tuple) ||
		    dt_util.get_datatype_num_constructors(tuple) != 1) {
			SET_ERROR_CODE(Z3_INVALID_ARG);
			RETURN_Z3(0);
		}
