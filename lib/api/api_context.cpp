...
namespace api {
	...
	void object::dec_ref() {
		...
		m_ref_count--;
		if (m_ref_count == 0) m_context.del_object(this);
	}
...
static char const *_get_error_msg(Z3_context c, Z3_error_code err) {
	switch (err) {
		...
		case Z3_PARSER_ERROR:
			return "parser error";
		case Z3_NO_PARSER:
			return "parser (data) is not available";
		...
		case Z3_DEC_REF_ERROR:
			return "invalid dec_ref command";
		case Z3_EXCEPTION:
			return c == 0 ? "Z3 exception" : mk_c(c)->get_exception_msg();
		default:
			return "unknown";
	}
}
